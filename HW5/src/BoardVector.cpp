...
namespace Okan{
	void HexVector::setSize(int set_size){
		size=set_size;
	}
	...
	int HexVector::isEnd_x(int check_count) const{
	...
	int HexVector::isEnd_o(int check_count) const{
	...
	void HexVector::upper(int check_count){
	...
	char HexVector::operator()(int row, int col) const{
		...
		try{
			if(row>=0 && col>=0 && row<size && col<size){
				...
				throw std::runtime_error("İnvalid Move");
			}
		}
